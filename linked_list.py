...
class LinkedListBasic:
	...
	def insert(self, i:int, newItem): 
		if i >= 0 and i <= self.__numItems:
			...
			newNode = ListNode(newItem, prev.next) #뉴노드는 newitem과 prev.next를 가진~
			prev.next = newNode # prev.next 는 newNode를 가르킨다.
			self.__numItems += 1
		else:
			print("index", i, ": out of bound in insert()") # 필요 시 에러 처리
 
	def append(self, newItem): #매개변수 newItem
		prev = self.__getNode(self.__numItems - 1) # __getNode()
